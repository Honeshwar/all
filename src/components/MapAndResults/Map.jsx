...
export default function Map() {
  ...
  const [viewport, setViewport] = useState({
    ...
    latitude: windowWidth < 640 ? 20.5937 : 20.5937 + 2,
    ...
    if (
      ...
      layers = [
        new GeoJsonLayer({
          ...
          extruded: false,
          pointType: "circle",
          lineWidthScale: 200, //electionType !== "STATE" ? 110 : 200,
          ...
          updateTriggers: {
            ...
            getLineColor: [mapResult],
            getLineWidth: [electionType],
            // lineWidthScale: [electionType],
          },
          transitions: {
            getFillColor: {
              duration: 2000,
    ...
    if (object && mapResult) {
      ...
      if (results && selected_Voter_Percentage.delta === default_delta_value) {
        ...
        for (let details of results.candidates) {
          ...
          modeled =
            modeled +
            ` <p class="flex items-center  gap-1 sm:gap-0  ">
            <span class="font-[600] text-[10px] min-w-[43px] flex sm:hidden justify-between items-center">
            ${details.party} <span class="pl-1">:</span>
          </span>
            <span class="text-[gray] text-[10px]   min-w-[55px] pl-1 sm:pl-0  ">${new Intl.NumberFormat(
              "en-IN"
        ...
        tooltipRef.current.innerHTML = `
          ...
          <div class="flex flex-col sm:flex-row gap-0  pt-0 pb-3 px-5  ">
          ...
          <div class="flex flex-col  pt-1    ">
          <span class="font-[600] pb-1 text-start sm:text-center text-sm mt-2 sm:mt-0">
            Modelled Vote
            <span class="px-2"></span>
          </span>
        <div class="flex flex-col gap-[7px] sm:border-l-[1px] border-gray-400  sm:pl-4 ">
        ${modeled}</div>
        </div>
          </div>
      ...
      setViewport(
        isRendering1
          ...
          : {
              longitude: windowWidth < 640 ? 78.9629 : 78.9629,
              latitude: windowWidth < 640 ? 20.5937 : 20.5937 + 2,
              zoom: windowWidth < 640 ? 2.500001 : 3.500001,
            }
      );
      setIsRendering1((p) => !p);
    } else {
      ...
      setViewport(
        isRendering2
          ...
          : {
              ...
              zoom:
                windowWidth < 800
                  ? stateCoordinates.zoom * 0.8200001
                  : stateCoordinates.zoom * 0.9800001,
            }
      );
      setIsRendering2((p) => !p);
    }
    ...
    <>
      ...
      {!isFetchingGeojson ? (
        <div
          ref={containerRef}
          id="react-map"
          className="election_calculator-tooltip w-[90%] sm:w-full h-[50vh]   lg:h-auto overflow-visible relative"
        >
          <DeckGL
            ref={deckRef}
